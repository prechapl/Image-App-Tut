...
export default class ImageOpsContainer extends React.Component {
  ...
  getSliderValue(key, type) {
    ...
    if (type === "rgb") {
      return this.getRBBCons().find(transform => transform.value === key)
        .default;
    } else if (type === "hsv") {
      return this.getHSVCons().find(transform => transform.value === key)
        .default;
    }
  }

  resetFilters(keys) {
    const newTransforms = this.state.transforms.filter(
                  ...
                  {this.getRBBCons().map(color => {
                    return (
                      <SliderComponent
                        ...
                        updateColorValue={(e, value, key) =>
                          this.updateColorValue(e, value, key)
                        }
                      />
                    );
                  })}
              ...
              <CardContent>
                <Box color="text.primary">
                  ...
                  {this.getHSVCons().map(color => {
                    return (
                      <SliderComponent
                        getSliderValue={key => this.getSliderValue(key, "hsv")}
                        default={0}
                        min={-100}
                        max={100}
                        keyLabel={color.key}
              ...
              <CardContent>
                <Box color="text.primary">
                  ...
                  <Button
                    variant="contained"
                    align="left"
                    onClick={() => this.createAdvanceEffects("oil_painting")}
                  >
                    Oil Painting
                  </Button>
      ...
      <div>
        ...
        <Slider
          ...
          onChangeCommitted={(e, value) =>
            this.props.updateColorValue(e, value, this.props.keyValue)
          }
        />
      </div>
    );
